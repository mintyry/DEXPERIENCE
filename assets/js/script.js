...
function renderPokemon(name) {
  ...
  fetch(pokeUrl)
    .then(function (response) {
      if (!response.ok) {
        body.setAttribute(
          "style",
          "background-image: url(./assets/images/city-landscapeglitch.webp);"
        );
        let errorMsg = `<p>Oak's words echoed... "There's a time and place for everything but not now!"</p>`;
        ...
        document.querySelector("#norris-quote").textContent =
          "That's not a Pokémon, LOL.";
        return;
      }
      document.querySelector("#norris-quote").textContent = "";
      body.setAttribute(
        "style",
        "background-image: url(./assets/images/city-landscape.webp);"
      );
  ...
  function renderAbilities(abilitiesArr) {
    ...
    for (let i = 0; i < abilitiesArr.length; i++) {
      HTML += `<span> ${
        abilitiesArr[i].ability.name.charAt(0).toUpperCase() +
  ...
  function renderTypes(typesArr) {
    ...
    for (let i = 0; i < typesArr.length; i++) {
      ...
      console.log(
        typesArr[0].type.name.charAt(0).toUpperCase() +
          typesArr[0].type.name.slice(1)
      );
    }
    return HTML;
  }
  ...
  function renderBaseStat(baseStatArr) {
    ...
    for (let i = 0; i < baseStatArr.length; i++) {
      HTML += `<li>${baseStatArr[i].stat.name.toUpperCase()}: ${
        baseStatArr[i].base_stat
      } </li>`;
    }
    return HTML;
  }
  ...
  function statCard(data) {
    ...
    statCardHTML +=
      ...
      `<div class="stat-element">
                <p><strong>NAME: </strong><span id = "squadName">${
                  data.name.charAt(0).toUpperCase() + data.name.slice(1)
                }</span></p><br>
                <p><strong>HEIGHT: </strong>${(
                  (data.height * 0.1 * 39.4) /
                  12
                ).toFixed(1)} ft</p><br>
               ...
               <p><strong>ABILITIES: </strong>${renderAbilities(
                 data.abilities
               )} </p><br>
               <p><strong>TYPES: </strong><span id = "squadType">${renderTypes(
                 data.types
               )}</span></p><br>
               <ul><strong>STATS: </strong>${renderBaseStat(
                 data.stats
               )}</ul><br>
            </div>`;
...
function addMySquad() {
  ...
  statContainer.addEventListener("dblclick", function (event) {
    ...
    addSquadPkmn.push(
      mySquadName.textContent.toLowerCase() + " - " + mySquadType.textContent
    );
...
function norrisFact(name) {
  ...
  fetch(catUrl, options)
    ...
    .then(function (data) {
      ...
      let pkmnQuote = cnQuote.replaceAll(
        /Chuck Norris/gi,
        pokeName.trim().charAt(0).toUpperCase() + pokeName.slice(1)
      );
      let pluralNorris = "Chuck Norris'";
      if (cnQuote.includes(pluralNorris)) {
        cnQuote.replaceAll(
          "Chuck Norris'",
          pokeName.trim().charAt(0).toUpperCase() + pokeName.slice(1) + `'s`
        );
      } else {
        document.querySelector(
          "#norris-quote"
        ).innerHTML = `<p style = "font-size: 4vh">Did you know?</p> <br> ${pkmnQuote}`;
      }
    });
